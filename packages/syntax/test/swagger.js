...
const immutableToJS = (value) => (isImmutable(value) ? value.toJS() : value)
...
function createObjWithHashedKeys(fdObj) {
  ...
  for (let pair of fdObj.entries()) {
    if (
      !newObj[pair[0]] &&
      !(trackKeys[pair[0]] && trackKeys[pair[0]].containsMultiple)
    ) {
 ...
 */
function mapToList(map, keyNames = 'key', collectedKeys = Im.Map()) {
  ...
  for (let entry of map.entries()) {
    let [key, val] = entry
    let nextList = mapToList(
      val,
      keyNames.slice(1),
      collectedKeys.set(keyName, key)
    )
...
// - else use ===
const propChecker = (props, nextProps, objectList = [], ignoreList = []) => {
  ...
  return (
    ...
    objectList.some(
      (objectPropName) => !eq(props[objectPropName], nextProps[objectPropName])
    )
  )
}
...
const validateMaxLength = (val, max) => {
  if (val.length > max) {
    return `Value must be no longer than ${max} character${
      max !== 1 ? 's' : ''
    }`
  }
}
...
const validateUniqueItems = (val, uniqueItems) => {
  ...
  if (uniqueItems === 'true' || uniqueItems === true) {
    ...
    if (hasDuplicates) {
      ...
      list.forEach((item, i) => {
        if (
          list.filter((v) => (isFunc(v.equals) ? v.equals(item) : v === item))
            .size > 1
        ) {
      ...
      if (errorsPerIndex.size !== 0) {
        return errorsPerIndex
          .map((i) => ({ index: i, error: 'No duplicates allowed.' }))
          .toArray()
      }
    }
  }
...
function validateValueBySchema(
  ...
  const requiresFurtherValidation =
    schemaRequiresValue || needsExplicitConstraintValidation || !isValidEmpty
  ...
  const noFurtherValidationNeeded =
    isValidNullable || !type || !requiresFurtherValidation
  ...
  let arrayListCheck =
    type === 'array' && Im.List.isList(value) && value.count()
  ...
  let objectStringCheck =
    type === 'object' && typeof value === 'string' && value
  ...
  if (
    ...
    if (
      schema &&
      schema.has('required') &&
      isFunc(requiredBySchema.isList) &&
      requiredBySchema.isList()
    ) {
...
const validateParam = (
  param,
  value,
  { isOAS3 = false, bypassRequiredCheck = false } = {}
) => {
  let paramRequired = param.get('required')

  let { schema: paramDetails, parameterContentMediaType } = getParameterSchema(
    param,
    { isOAS3 }
  )

  return validateValueBySchema(
    value,
    paramDetails,
    paramRequired,
    bypassRequiredCheck,
...
function requiresValidationURL(uri) {
  if (
    !uri ||
    uri.indexOf('localhost') >= 0 ||
    uri.indexOf('127.0.0.1') >= 0 ||
    uri === 'none'
  ) {
    return false
  }
...
const getCommonExtensions = (defObj) =>
  defObj.filter((v, k) =>
    /^pattern|maxLength|minLength|maximum|minimum/.test(k)
  )
...
function paramToIdentifier(
  param,
  { returnAll = false, allowHashes = true } = {}
) {
  ...
  if (param && param.hashCode && paramIn && paramName && allowHashes) {
    generatedIdentifiers.push(
      `${paramIn}.${paramName}.hash-${param.hashCode()}`
    )
  }
